...
class parameter_adjusting:
    ...
    def __init__(self, path_parameters_folder):
        self.parameters_folder = path_parameters_folder  # Folder to store parameter files and images
        ...
        try:
            ...
            with open(f"{self.parameters_folder}/points.txt", 'w') as file:
                file.write("")
    ...
    def capture_template(self, camera):
        ...
        cv2.imwrite(f"{self.parameters_folder}/template.jpg", dst)
        print("template.jpg saved")
        cv2.destroyAllWindows()

    def adjust_parameters(self, folder_path, camera):
        """
        ...
        - Template capture
        """
        self.parameters_folder = folder_path
        end = False
        try:
            while not end:
