...
class CameraCalibrator:
    def __init__(self):
        ...
        if not os.path.exists(self.corner_folder):
            os.makedirs(self.corner_folder)
        ...
        with open(file_path, 'r') as file:
            for i, line in enumerate(file, 1):
                if i <= 4:
                    ...
                    self.shape= map(int, line.strip().split(','))
        return points
    ...
    def on_EVENT_LBUTTONDOWN(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN and self.frame is not None and (flags & cv2.EVENT_FLAG_CTRLKEY):
            ...
            cv2.putText(self.display_frame, xy, (x+10, y+10), cv2.FONT_HERSHEY_PLAIN,
                        3.0, (0,0,0), thickness=3)
            print("x:{},y:{}".format(x, y))
            bgr_list = self.frame[y, x]
            rgb_list = bgr_list[::-1]
            print("RGB为: ", rgb_list)
            print("十六进制颜色：", self.rgb2hex(rgb_list))
            print("--" * 20)
    ...
    def capture_image(self, camera):
        ...
        filename = f'{self.corner_folder}/p1.jpg'
                    ...
                    if self.frame is not None:
                        ...
                        while True:
                            ...
                            if len(self.points) == 4:
                                ...
                                cv2.imwrite(f'{self.corner_folder}/output.jpg', dst)
                                self.shape = dst.shape
                                self.save_points_to_file(self.points, f'{self.corner_folder}/points.txt')
                                loaded_points = self.load_points_from_file(f'{self.corner_folder}/points.txt')
        ...
        try:
            while True:
                ...
                if mode == '0':
                    ...
                    with open("/corner/real_size.txt", 'w') as file:
                        file.write(f"{x},{y}\n")
                elif mode == '2':
